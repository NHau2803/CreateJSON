...
contentOfDeadline = ["Nộp lần 1", "Nộp bài lần 1", "Nộp phần 1", "Nộp bài tập", "Nộp chương đầu", "Nộp bài tập lớn", "Kiểm tra tiến độ", "Kiểm tra lần 2", 
"Nộp chương 2", "Nhớ nộp bài", "nộp bài nha mấy bạn!", "hạn chót", "phần 4", "phần 7", "phần 6", "Nộp lấy điểm cộng",
...
lastNames = { 
    "female":["Thị","Kim","Vân","Vĩ","Bảo","Mỹ","Huyền","Ngọc","Tú","Thùy","Mai","Diễm","Hồng","Anh","Tú","Quỳnh","Trang","Huyền","Lan","Hương","Trúc"] , 
    "male":["Trí","Công","Văn","Hùng","Minh","Việt","Trọng","Thành","Phương","Chí","Trung","Hậu","Hưng","Nhân","Khải","Khánh","Duy","Lợi","Quân"] }
...
def jsonStudent(code, name, gender, birthday, email, phone, facultyId, username, password):
    return '{ "code":"%s", "name":"%s", "gender":"%s", "birthday":"%s", "email":"%s", "phone":"%s", "facultyId":%d,"username":"%s","password":"%s" }%s'%(code, name, gender, birthday, email, phone, facultyId,username,password,",")

def jsonTeacher(code, name, gender, birthday, email, phone, academyId, positionId, facultyId, username, password):
    return '{ "code":"%s", "name":"%s", "gender":"%s", "birthday":"%s", "email":"%s", "phone":"%s", "academyId":%d,"positionId":%d,"facultyId":%d,"username":"%s","password":"%s" }%s'%(code, name, gender, birthday, email, phone, academyId, positionId,facultyId,username,password,",")

def jsonTopic(code, name, startTime, endTime, teacherId, typeTopicId, deadlines):
    return '{ "code":"%s", "name":"%s", "startTime":"%s", "endTime":"%s", "teacherId":%d, "typeTopicId":%d, "deadlines":%s}%s'%(code, name, startTime, endTime, teacherId, typeTopicId, deadlines,",")
...
def runForTeacher(limit):
    ...
    for i in range(limit):
        ...
        jsonContent += jsonTeacher(code, name, gender, birthday, email, phone, academyId, positionId, facultyId, username, password)
    return '[%s]'%(jsonContent)
...
def runForTopic(limit):
    ...
    for i in range(limit):
        ...
        for i in range(0, lenDateTime-1):
            deadlineContent += '"%s",'%(str(dateTime[i]+"**"+dateTime[i+1]+"**"+str(random.choice(contentOfDeadline))))
